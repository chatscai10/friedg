...
const PosOrderPanel: React.FC = () => {
  ...
  const { loading: permissionsLoading, permissions } = useSpecialPermissions();
  ...
  // 處理應用折扣
  const handleApplyDiscount = (discountData: { type: 'percentage' | 'amount', value: number }) => {
    // 確保 applyDiscount 在 PosOrderContext 中被實現
    applyDiscount(discountData);
    
    // 顯示成功訊息
    showMessage(`已成功套用${discountData.type === 'percentage' ? `${discountData.value}%` : `${discountData.value}元`}的折扣`, 'success');
  };
  ...
  return (
    <Paper elevation={2} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <PosDiscountDialog
        ...
        orderId={checkoutState.orderId || undefined}
      />
