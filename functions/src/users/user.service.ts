 ...
 * @param userId 用戶ID
 * @param status 新的用戶狀態
 * @param tenantId 租戶ID (用於權限檢查)
 * @returns 更新後的用戶資料或錯誤信息
 */
export async function updateUserStatus(userId: string, status: string, tenantId: string): Promise<{ data?: any; error?: string; code?: string }> {
  logger.info(`開始更新用戶狀態，UID: ${userId}`, { status });
  ...
  try {
    ...
    if (!userDoc.exists) {
      logger.warn(`嘗試更新不存在的用戶狀態，UID: ${userId}`);
      return { 
        error: `用戶不存在: ${userId}`, 
        code: 'USER_NOT_FOUND' 
      };
    ...
    if (!userData) {
      ...
      return { 
        error: `用戶數據為空: ${userId}`, 
        code: 'USER_DATA_EMPTY' 
      };
    }
    
    // 2. 檢查租戶隔離 (確保只有相同租戶或系統管理員可以操作)
    if (userData.tenantId !== tenantId && tenantId !== 'system') {
      logger.warn(`租戶隔離檢查失敗，用戶租戶: ${userData.tenantId}, 請求租戶: ${tenantId}`);
      return { 
        error: '無權操作其他租戶的用戶', 
    ...
    // 3. 更新 Firestore 中的用戶狀態
    await db.collection('users').doc(userId).update({
      status: status,
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
    
    // 4. 如果狀態為 INACTIVE 或 SUSPENDED，則禁用 Firebase Auth 帳戶
    // 如果狀態為 ACTIVE，則啟用帳戶
    let disabled = false;
    if (status === 'inactive' || status === 'suspended' || status === 'deleted') {
      disabled = true;
    }
    
    // 更新 Firebase Auth 帳戶狀態
    ...
    logger.info(`成功更新用戶狀態，UID: ${userId}`, { status, disabled });
    ...
    logger.error(`更新用戶狀態時發生錯誤，UID: ${userId}`, { error: error.message, stack: error.stack });
    return {
      error: `更新用戶狀態失敗: ${error.message}`,
      code: 'INTERNAL_ERROR'
    };
  }
} 