...
import { BusinessHours, DailyOperatingHours, Coordinates } from '../types/core-params';
...
export class AttendanceService {
  ...
  public static async clockIn(
    ...
    try {
      ...
      // 4. 檢查營業時間
      const isWithinBusinessHours = this.checkBusinessHours(currentDate, storeInfo.businessHours);
      ...
      // 有排班資訊，判斷是否遲到
        ...
        const minutesLate = (currentDate.getTime() - scheduledStartTime.getTime()) / (1000 * 60);
      ...
      const attendanceLog: AttendanceLog = {
        ...
        storeId,
        date,
        clockInTime: firestoreTimestamp,
        clockInCoords: {
        ...
        clockInDeviceInfo: request.deviceInfo,
  ...
  public static async clockOut(
    ...
    try {
      ...
      // 有排班資訊，判斷是否早退
        ...
        const minutesEarly = (scheduledEndTime.getTime() - currentDate.getTime()) / (1000 * 60);
      ...
      // 9. 計算工作時間（分鐘）
      const clockInDate = clockInRecord.clockInTime.toDate();
      const nowDate = new Date();
      const workDurationMinutes = Math.round((nowDate.getTime() - clockInDate.getTime()) / (1000 * 60));

      // 10. 更新出勤記錄
      const firestoreTimestamp = admin.firestore.Timestamp.now();
      const updatedAttendanceLog: Partial<AttendanceLog> = {
        clockOutTime: firestoreTimestamp,
        clockOutCoords: {
          latitude: request.latitude,
          longitude: request.longitude
        },
