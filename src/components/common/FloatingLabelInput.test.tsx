...
describe('FloatingLabelInput', () => {
  ...
  it('用戶輸入時更新值', async () => {
    ...
    renderWithTheme(<FloatingLabelInput label="使用者名稱" onChange={handleChange} />);
  ...
  // 測試初始值是否正確設置
  it('初始值正確設置', () => {
    renderWithTheme(<FloatingLabelInput label="電子郵件" value="initial@example.com" />);
  ...
  // 測試錯誤狀態
  it('顯示錯誤狀態和輔助文字', () => {
    renderWithTheme(<FloatingLabelInput label="用戶名" error={true} helperText="用戶名已被使用" />);
  ...
  // 測試必填屬性
  it('顯示必填標記', () => {
    renderWithTheme(<FloatingLabelInput label="必填欄位" required={true} />);
