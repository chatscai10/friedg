...
// 其他導入
import { onRequest } from "firebase-functions/v2/https"; // Gen 2 import for HTTP functions
import { logger } from "firebase-functions/v2"; // Gen 2 logger
...
if (process.env.FUNCTIONS_EMULATOR === 'true') {
  ...
  process.env.FIRESTORE_EMULATOR_HOST = 'localhost:8080'; // 保持 Firestore 模擬器主機設置
  process.env.FIREBASE_AUTH_EMULATOR_HOST = 'localhost:9099'; // 設置 Auth 模擬器主機，Admin SDK 會自動使用
  // admin.auth().emulatorConfig = { // 這一行和下面的對象將被移除，因為已棄用且導致錯誤
  //   host: 'localhost',
  //   port: 9099
  // };
  console.log(`Auth emulator host set to: ${process.env.FIREBASE_AUTH_EMULATOR_HOST}`);
}
...
app.get('/v1/ping', (req, res) => {
  logger.info('Ping received!'); // Gen 2 logger
...
// export const api = functions.https.onRequest(app); // Gen 1 export
export const api = onRequest(app); // Gen 2 export

// 其他導出也需要更新為 Gen 2 風格，此處僅為示例
// 例如:
// import { onCall } from "firebase-functions/v2/https";
// export const someCallable = onCall((request) => { /* ... */ });

// import { onDocumentWritten } from "firebase-functions/v2/firestore";
// export const firestoreTrigger = onDocumentWritten("documents/{docId}", (event) => { /* ... */ });

// 導入股權模塊函數 (假設這些已經是 Gen2 或者不需要 Firebase Functions 包裹)
export { checkEquityEligibility, openPurchaseWindow, closePurchaseWindow, revalueShares, autoDistributeDividends, processInstallmentDebit } from './equity';

// 導入財務模塊函數 (假設這些已經是 Gen2 或者不需要 Firebase Functions 包裹)
export { calculateMonthlyProfit, generateMonthlyProfitReports } from './financial';

// 導入支付模塊 (假設這些已經是 Gen2 或者不需要 Firebase Functions 包裹)
export * as payments from './payments'; // 如果 payments 內部有 Gen1 函數，也需更新

// 導入訂單模塊的所有Cloud Functions (這些都需要檢查並更新為 Gen2)
// 例如，如果 getOrders 是 HTTP 函數:
// export const getOrders = onRequest(async (req, res) => { /* ... */ });
// 如果是 onCall:
// export const getOrders = onCall(async (request) => { /* ... */ });
export * from './orders'; // 這行可能需要逐個導出並轉換

// 導出通知模塊的Cloud Functions (這些都需要檢查並更新為 Gen2)
export * from './notifications'; // 這行可能需要逐個導出並轉換

// 導出訂單狀態變更通知觸發器 (onOrderUpdate 來自 Gen1 orders.triggers, 也需更新)
// import { onDocumentUpdated } from "firebase-functions/v2/firestore";
// export const orderStatusChangeNotification = onDocumentUpdated("orders/{orderId}", onOrderUpdate);
// (注意: onOrderUpdate 函數本身也需要適應 V2 event 結構)

// 清理日誌 (這個示例也改為 Gen 2)
export const cleanupLogs = onRequest((req, res) => { // Gen 2
  logger.info('接收到日誌清理請求'); // Gen 2 logger
  res.status(200).json({
    status: 'success',
