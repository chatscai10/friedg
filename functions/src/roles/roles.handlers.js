...
const { logAction, AuditAction } = require('../libs/audit'); // 引入審計日誌模組
  ...
  try {
      ...
      try {
        await logAction({
          ...
          details: { 
            ...
            reason: '未授權：用戶角色無權創建角色' 
          },
          status: 'failure',
          errorMessage: '未授權：只有 SuperAdmin 和 TenantAdmin 可以創建角色',
          ipAddress: req.ip,
    ...
    if (!validationResult.success) {
      ...
      try {
        await logAction({
          ...
          status: 'failure',
          errorMessage: `請求參數錯誤：${validationResult.error.errors[0].message}`,
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        });
      } catch (auditError) {
    ...
    try {
      await logAction({
        ...
        userName: req.user ? (req.user.name || req.user.displayName) : 'unknown',
        ...
        status: 'failure',
        errorMessage: '系統內部錯誤: ' + error.message,
        ipAddress: req.ip,
        userAgent: req.headers['user-agent']
      });
    } catch (auditError) {
...
exports.updateRole = async (req, res) => {
  try {
    ...
    if (userRole === "super_admin") {
      ...
      try {
        await logAction({
          ...
          details: { 
            requestBody: req.body,
            reason: '未授權：用戶角色無權更新角色' 
          },
          status: 'failure',
          errorMessage: '未授權：只有 SuperAdmin 和 TenantAdmin 可以更新角色',
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        });
      } catch (auditError) {
    ...
    if (!roleDoc.exists) {
      ...
      try {
        await logAction({
          ...
          status: 'failure',
          errorMessage: `找不到 ID 為 ${roleId} 的角色`,
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        });
      } catch (auditError) {
    ...
    // 4. 權限檢查：租戶隔離
    if (roleData.tenantId && userRole !== "super_admin" && roleData.tenantId !== userTenantId) {
      // 非 SuperAdmin 用戶不能更新其他租戶的角色
      console.warn(`用戶 ${requestingUser.uid} (${userRole}) 嘗試更新租戶 ${roleData.tenantId} 的角色，但其屬於租戶 ${userTenantId}`);
      ...
      try {
        await logAction({
          ...
          details: { 
            requestBody: req.body,
            roleTenantId: roleData.tenantId,
            reason: '未授權：跨租戶操作被拒絕' 
          },
          status: 'failure',
          errorMessage: '未授權：您無權更新此角色',
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        });
      } catch (auditError) {
    ...
    // 5. 權限檢查：全局角色限制
    if (roleData.tenantId === null && userRole !== "super_admin") {
      // 非 SuperAdmin 用戶不能更新全局角色
      console.warn(`用戶 ${requestingUser.uid} (${userRole}) 嘗試更新全局角色`);
      ...
      try {
        await logAction({
          ...
          details: { 
            requestBody: req.body,
            isGlobalRole: true,
            reason: '未授權：只有超級管理員可以更新全局角色' 
          },
          status: 'failure',
          errorMessage: '未授權：只有超級管理員可以更新全局角色',
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        });
      } catch (auditError) {
      ...
      return res.status(403).json({
        status: "error",
        errorCode: "E403",
        message: "未授權：只有超級管理員可以更新全局角色"
      });
    }
    
    // 獲取要更新的資料
    const { roleName, description, level, permissions, isActive } = req.body;
      ...
      if (!permissionValidation.valid) {
        ...
        try {
          await logAction({
            ...
            details: { 
              requestBody: req.body,
              validationError: permissionValidation.error,
              reason: '無效的權限格式' 
            },
            status: 'failure',
            errorMessage: permissionValidation.error,
            ipAddress: req.ip,
            userAgent: req.headers['user-agent']
          });
        } catch (auditError) {
