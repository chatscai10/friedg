...
// Add other order handlers here (create, update, etc.) 
  ...
  try {
    ...
    for (const item of items) {
      if (!item.menuItemId || !item.quantity || item.quantity <= 0) {
        return res.status(400).send({ message: `Invalid item data: Each item requires menuItemId and positive quantity. Problem: ${JSON.stringify(item)}` });
      }
      ...
      if (!menuItemSnap.exists || !menuItemSnap.data().isActive || menuItemSnap.data().storeId !== storeId) {
        return res.status(400).send({ message: `Invalid or unavailable menu item: ${item.menuItemId} for store ${storeId}.` });
      }
      ...
      if (item.options && Array.isArray(item.options)) {
        for (const option of item.options) {
          if (!option.menuOptionId) {
            return res.status(400).send({ message: `Invalid option data for item ${item.menuItemId}: menuOptionId is required.` });
          }
          ...
          if (!menuOptionSnap.exists || !menuOptionSnap.data().isActive || menuOptionSnap.data().menuItemId !== item.menuItemId) {
            return res.status(400).send({ message: `Invalid or unavailable menu option: ${option.menuOptionId} for item ${item.menuItemId}.` });
          }
      ...
      if (finalItemPrice < 0) {
         console.warn(`Calculated negative price for item ${item.menuItemId}. Clamping to 0.`);
         finalItemPrice = 0;
      }
      ...
      validatedItems.push({
        ...
        unitPrice: finalItemPrice, // Calculated unit price after options
    ...
    if (error.message.startsWith("Invalid")) { // Basic check
...
exports.updateOrderStatus = async (req, res) => {
  ...
  if (!status || !VALID_ORDER_STATUSES.includes(status)) {
    return res.status(400).send({ message: `Invalid status provided. Must be one of: ${VALID_ORDER_STATUSES.join(", ")}.` });
  }

  // Check admin roles
  if (requestingUser.role !== "TenantAdmin" && requestingUser.role !== "StoreManager") {
    return res.status(403).send({ message: "Forbidden: Only admins or store managers can update order status." });
  }
  ...
  try {
    ...
    if (!canUpdate) {
      ...
      return res.status(403).send({ message: `Forbidden: You do not have permission to update the status for order ${orderId}.` });
    }
    ...
    res.status(500).send({ message: "Failed to update order status due to an internal error.", error: error.message });
  }
};
...
exports.getOrderById = async (req, res) => {
  ...
  try {
    ...
    if (!canView) {
        ...
        return res.status(403).send({ message: "Forbidden: You do not have permission to view this order." });
    }
    ...
    res.status(500).send({ message: "Failed to fetch order details.", error: error.message });
  }
};
...
exports.updateOrder = async (req, res) => {
  ...
  if (deliveryInfo === undefined && notes === undefined) {
      return res.status(400).send({ message: "No valid fields provided for update. Only 'deliveryInfo' and 'notes' can be updated." });
  }
  ...
  try {
    ...
    if (orderData.status !== "pending") {
        return res.status(403).send({ message: `Forbidden: Order can only be updated when status is 'pending'. Current status: ${orderData.status}` });
    }
    ...
    res.status(500).send({ message: "Failed to update order due to an internal error.", error: error.message });
  }
};
...
exports.recordPayment = async (req, res) => {
  ...
  if (!orderId || !paymentMethod || amount === undefined || amount <= 0) {
    return res.status(400).send({ message: "Missing required fields: orderId, paymentMethod, and positive amount are required." });
  }
  ...
  try {
    ...
    if (!canRecord) {
      ...
      return res.status(403).send({ message: "Forbidden: You do not have permission to record payments for this order." });
    }
    ...
    res.status(500).send({ message: "Failed to record payment due to an internal error.", error: error.message });
  }
};
...
exports.getOrderStatistics = async (req, res) => {
  ...
  if (requestingUser.role === "StoreManager" && requestingUser.storeId !== targetStoreId) {
      return res.status(403).send({ message: "Forbidden: StoreManager can only view statistics for their assigned store." });
  }
  ...
  try {
    ...
    res.status(500).send({ message: "Failed to fetch order statistics due to an internal error.", error: error.message });
  }
};

/**
 * Handler to trigger the generation of an order receipt.
 * Permissions: TenantAdmin, StoreManager (managing the order's store).
 * TODO: Implement actual receipt generation logic (e.g., PDF generation, storing URL).
 * @param {import("express").Request} req Express request object.
 * @param {import("express").Response} res Express response object.
 * @returns {Promise<void>} A promise that resolves when the response is sent.
 */
exports.generateOrderReceipt = async (req, res) => {
  ...
  // Check admin roles
  if (requestingUser.role !== "TenantAdmin" && requestingUser.role !== "StoreManager") {
    return res.status(403).send({ message: "Forbidden: Only admins or store managers can generate receipts." });
  }
  ...
  try {
    ...
    if (!canGenerate) {
      ...
      return res.status(403).send({ message: "Forbidden: You do not have permission to generate receipts for this order." });
    }
    ...
    res.status(500).send({ message: "Failed to generate receipt due to an internal error.", error: error.message });
  }
};

/**
 * Handler to get the receipt information for an order.
 * Permissions: Requester must be the user who placed the order OR TenantAdmin/StoreManager with permission.
 * @param {import("express").Request} req Express request object.
 * @param {import("express").Response} res Express response object.
 * @returns {Promise<void>} A promise that resolves when the response is sent.
 */
exports.getOrderReceipt = async (req, res) => {
  ...
  try {
    ...
    if (!canView) {
      console.warn(`User ${requestingUser.uid} attempt to get receipt for order ${orderId} denied.`);
      return res.status(403).send({ message: "Forbidden: You do not have permission to view the receipt for this order." });
    }

    // 3. Check if receipt exists and return it
    if (orderData.receiptUrl) {
      // Option 1: Return the URL
      res.status(200).send({ 
          orderId: orderId, 
          receiptUrl: orderData.receiptUrl, 
          generatedAt: orderData.receiptGeneratedAt || null 
      });
      // Option 2: Redirect to the URL (if it's a direct link)
      // return res.redirect(orderData.receiptUrl);
      // Option 3: Fetch the receipt content and return it (if stored elsewhere)
      // ... fetch content ...
      // res.status(200).contentType('application/pdf').send(receiptContent);
    } else {
      return res.status(404).send({ message: "Receipt not generated or available for this order." });
    }

  } catch (error) {
    console.error(`Error fetching receipt for order ${orderId}:`, error);
    res.status(500).send({ message: "Failed to fetch order receipt due to an internal error.", error: error.message });
  }
}; 