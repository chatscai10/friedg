...
export const withAuthentication = async (
  ...
  try {
    // 從請求頭部獲取 Authorization
    const authHeader = req.headers.authorization;
    if (!authHeader) {
      console.error('未提供 Authorization 頭部');
      return res.status(401).json({
        status: 'error',
        message: '未授權：請提供有效的身份驗證令牌'
      });
    }
    ...
    // 檢查自定義聲明 (Custom Claims)
    const { uid, email, role, roleLevel, tenantId, storeId, additionalStoreIds } = decodedToken;
    
    // 驗證關鍵聲明
    if (!role || !tenantId) {
      console.error(`用戶 ${uid} 缺少關鍵自定義聲明：role=${role}, tenantId=${tenantId}`);
      return res.status(403).json({
        status: 'error',
        message: '未授權：用戶令牌缺少必要的權限信息'
      });
    }
    ...
    const user: UserContext = {
      ...
      email,
      role,
      roleLevel: roleLevel || 10, // 預設最低權限等級
      tenantId,
      storeId,
      additionalStoreIds: Array.isArray(additionalStoreIds) ? additionalStoreIds : []
    };
