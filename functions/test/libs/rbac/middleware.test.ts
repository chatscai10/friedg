...
import { CallableContext } from '../../../src/libs/rbac/types';
import { UserInfo } from '../../../src/libs/rbac/types';

// 模擬 getUserInfoFromClaims
jest.mock('../../../src/libs/rbac', () => ({
  getUserInfoFromClaims: jest.fn()
}));
import { getUserInfoFromClaims } from '../../../src/libs/rbac';
...
describe('RBAC - 身份驗證中間件', () => {
  ...
  describe('withRole', () => {
    ...
    it('應該拒絕無效角色類型的請求', async () => {
      ...
      await expect(enhancedHandler({}, mockContext))
        .rejects.toThrow(`角色類型 'invalid_role' 無效`);
