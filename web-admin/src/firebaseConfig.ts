...
if ((isDevelopment || isUsingEmulator) && !isAuthEmulatorConnected) {
  try {
    ...
    // 使用正確的端口 9099，這是 Firebase Auth 模擬器的默認端口
    auth.useEmulator("http://127.0.0.1:9099");
    // 如果需要其他模擬器，請取消註解以下行
    // firestore?.useEmulator('127.0.0.1', 9090);
    // functions?.useEmulator('127.0.0.1', 5003);
    isAuthEmulatorConnected = true;
    console.log("Firebase 模擬器已連接。端口: auth=9099");
  } catch (error) {
...
export { app, auth }; // 導出必要的實例
// export { app, auth, firestore, functions }; // 若需要全部導出請取消註解 