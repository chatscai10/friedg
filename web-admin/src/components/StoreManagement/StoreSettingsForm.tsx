...
import { Store, StoreOperatingHours, StoreGpsFence, StorePrinterSettings, UpdateStoreBusinessHoursPayload } from '../../types/store';
import { BusinessHours, convertArrayToBusinessHours, convertBusinessHoursToArray } from '../../../src/types/core-params';
...
const StoreSettingsForm: React.FC<StoreSettingsFormProps> = ({ storeId, onSuccess, onCancel }) => {
    ...
    const fetchStoreData = async () => {
      ...
      try {
        ...
        if (!storeData.operatingHours || storeData.operatingHours.length < 7) {
          ...
          // 嘗試從 businessHours 轉換
          if (storeData.businessHours) {
            defaultHours.push(...convertBusinessHoursToArray(storeData.businessHours));
          } else {
            // 如果沒有營業時間，創建默認值
            for (let day = 0; day <= 6; day++) {
              const existingDayHours = storeData.operatingHours?.find(hours => hours.day === day);
              
              defaultHours.push({
                day,
                isOpen: existingDayHours?.isOpen || (day >= 1 && day <= 5), // 週一至週五預設開業
                openTime: existingDayHours?.openTime || '09:00',
                closeTime: existingDayHours?.closeTime || '18:00'
              });
            }
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 處理營業時間，將陣列轉換為標準 BusinessHours 格式
      const businessHours = convertArrayToBusinessHours(formData.operatingHours || []);
      ...
      const businessHoursPayload: UpdateStoreBusinessHoursPayload = {
        businessHours
      };
      ...
      await Promise.all([
        ...
        updateStore(storeId, operatingHoursPayload),
        updateStore(storeId, businessHoursPayload) // 新增支援標準格式
      ]);
